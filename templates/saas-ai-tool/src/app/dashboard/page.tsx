    ...
    <div>
      ...
      {error && (
        <div className="mt-4 rounded-lg p-4" style={{ background: "rgba(239,68,68,0.1)", border: "1px solid rgba(239,68,68,0.2)" }}>
          ...
          <Link href="/dashboard/billing" className="mt-2 inline-block text-xs text-violet-400 hover:text-blue-300">Upgrade Plan</Link>
        </div>
      )}
        ...
        {items.length === 0 ? (
          ...
          items.map(item => (
            <Link key={item.id as string} href={`/dashboard/${item.id}`}>
              <div className="flex items-center justify-between rounded-xl p-4 transition-all hover:border-violet-500/30" style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)" }}>
                <div>
